...
function App() {
  ...
  return (
    <>
      <div className={`min-h-screen flex items-center flex-col bg-amber-50 dark:bg-gray-900 transition transition-all duration-200 ${modo}`}>
        <Header modo={modo} setModo={setModo}></Header>
        <div className="w-9/10 flex sm:flex-row flex-col justify-end items-center mt-5 gap-5">
          <div className="flex justify-center items-center relative sm:w-auto w-full">
            <SlMagnifier className="absolute left-2 text-lg dark:text-white z-2"/>
            <input type="text" 
              className="pr-2 pl-8 sm:text-lg text-md bg-white dark:bg-black dark:text-white rounded border border-gray-800 focus:outline-none focus:ring-2 focus:ring-rose-600 focus:border-rose-600 w-full z-1" 
              value={busca} 
          ...
          <select 
            className="bg-white dark:bg-black dark:text-gray-200 pr-5 pl-1 sm:text-lg text-md rounded border border-gray-800 sm:w-auto w-full" 
        ...
        <button className={`${usuariosFiltrados.length === 0 ? "hidden" : "flex"} flex-col justify-center items-center mt-5 px-4 py-2 hover:scale-125 trasition-all duration-200 dark:text-white`} onClick={() => {usuariosFiltrados.length > mostrados ? setMostrados(mostrados + 10) : setMostrados(10)}}>
          <span>{usuariosFiltrados.length > mostrados ? "Mostrar mais" : "Mostrar Menos"}</span>
          <MdKeyboardArrowDown className={`${usuariosFiltrados.length > mostrados ? "" : "rotate-180"}`}/>
        </button>
        <Footer></Footer>
      </div>
    </>
